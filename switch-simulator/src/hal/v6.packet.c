/**
 * @file packet.c
 * @brief Расширенная реализация интерфейса обработки пакетов для симулятора коммутатора
 * 
...
/**
 * @brief Освободить все буферы в пуле
 */
static void destroy_buffer_pool(void) {
    pthread_mutex_lock(&g_buffer_pool.mutex);
    
    for (uint32_t i = 0; i < g_buffer_pool.count; i++) {
        if (g_buffer_pool.buffers[i]->data) {
            free(g_buffer_pool.buffers[i]->data);
        }
    ...
    LOG_INFO(LOG_CATEGORY_HAL, "Пул буферов пакетов освобожден");
}
...
status_t packet_init(void) {
    ...
    if (g_initialized) {
        LOG_WARNING(LOG_CATEGORY_HAL, "Подсистема обработки пакетов уже инициализирована");
        return STATUS_ALREADY_INITIALIZED;
    }   
    
    // Инициализировать мьютекс для списка обработчиков
    if (pthread_mutex_init(&g_processors_mutex, NULL) != 0) {
        LOG_ERROR(LOG_CATEGORY_HAL, "Не удалось инициализировать мьютекс обработчиков пакетов");
        return STATUS_FAILURE;
    }   
    
    // Инициализировать мьютекс для статистики
    if (pthread_mutex_init(&g_packet_stats.mutex, NULL) != 0) {
        LOG_ERROR(LOG_CATEGORY_HAL, "Не удалось инициализировать мьютекс статистики пакетов");
        pthread_mutex_destroy(&g_processors_mutex);
        return STATUS_FAILURE;
    }   
    
    // Очистить все слоты обработчиков
    memset(g_processors, 0, sizeof(g_processors));
    g_processor_count = 0;
    
    // Очистить статистику
    memset(&g_packet_stats, 0, sizeof(g_packet_stats));
    
    // Инициализировать пул буферов
    status_t status = init_buffer_pool();
    if (status != STATUS_SUCCESS) {
        LOG_ERROR(LOG_CATEGORY_HAL, "Не удалось инициализировать пул буферов пакетов");
        pthread_mutex_destroy(&g_processors_mutex);
        pthread_mutex_destroy(&g_packet_stats.mutex);
        return status;
    }   
    
    g_initialized = true;
    
    LOG_INFO(LOG_CATEGORY_HAL, "Подсистема обработки пакетов успешно инициализирована");
    return STATUS_SUCCESS;
}
